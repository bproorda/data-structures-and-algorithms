...
const filterStringsWithVowels = (arr) => {
    let myRegex = /\w*[aeiou]\w*/ig
    return arr.filter(string => {
    let filteredString =  string.match(myRegex);
    if(filteredString) {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(pokeStat => {
    return pokeStat.baseStat > minBaseStat;
  });
};
...
//   result = [];
//    arr.filter(pokeStat => {
//     if (pokeStat.baseStat > minBaseStat) {
//       result.push(pokeStat.stat.name);
//     }
//   });
//   return result;
// };
...
const evenOddNumericValues = (arr) => {
  return arr.filter(item =>{return (((typeof item) !== 'string') ? !isNaN(item) : false);}).map(item2 => {return ((item2 % 2 === 0) ? 'even' : 'odd' )})};
