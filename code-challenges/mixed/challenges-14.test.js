...
const toTitleCase = (arr) => {
    return arr.map( item =>  item.charAt(0).toUpperCase()+item.substring(1, item.length + 1))
  };
...
let biggerThanLuke = (arr) => {
    let filtered = arr.filter(character => {
      return parseInt(character.mass) > parseInt(arr[0].mass);
    });
    let names = filtered.map(character => {
