...
const listFoods = (recipe) => {
    ...
    for (let i = 0; i < recipe.ingredients.length; i++ ) {
        ...
        for( let j = 0; j < ingredient.length; j++) {
            let letter = ingredient.slice(j, j+1);
            if (letter === ' ') {
                spaceCount++;
            }
            if(spaceCount == 2) {
              newStr = ingredient.slice(j+1, ingredient.length);
              break;
            }
        }
...
const removeVowels = (str) => {
  let newArr = str.split('');
    let newStr = newArr.filter(element => !vowels.includes(element)).join('');
...
const extractVowels = (str) => {
  ...
  let newArr = str.split('');
  let newStr = newArr.filter(element => !vowels.includes(element)).join('');
  let vowelStr = newArr.filter(element => vowels.includes(element)).sort().join('');
  result.push(newStr, vowelStr);
  return result;
};
